...
PROJECT="aewan-1.0.01"

if [ ! -d "./$PROJECT" ]; then
	>&2 echo "Please install the 'libncurses5-dev' package to build:"
       	>&2 echo "sudo apt-get install libncurses5-dev"

	tar zxvf downloaded/$PROJECT.tar.gz
	pushd $PROJECT
	if [ $? -eq 1 ]; then
