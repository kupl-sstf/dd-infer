...
if [ ! -d "./$PROJECT" ]; then
	>&2 echo "Please install the 'asciidoc' package to build:"
       	>&2 echo "sudo apt-get install asciidoc"
	wget https://www.coin-or.org/download/source/DyLP/DyLP-1.10.4.tgz
	tar zxvf DyLP-1.10.4.tgz
	rm DyLP-1.10.4.tgz
	pushd $PROJECT
	if [ $? -eq 1 ]; then
