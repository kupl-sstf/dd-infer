...
def train ():
    ...
    train_and_save_model (filename_train_pos, filename_train_neg, savefile)

def train_and_save_model (filename_train_pos, filename_train_neg, savefile):
    start = time.time() 
    train_pos = pandas.read_csv(filename_train_pos, header=None, delimiter=r"\s+", dtype=np.int16).values
    train_neg = pandas.read_csv(filename_train_neg, header=None, delimiter=r"\s+", dtype=np.int16).values

    print ("---- training set ----")
    print ("#positive examples", len(train_pos))
    print ("#negative examples", len(train_neg), flush=True)
    weight = len(train_neg) / len(train_pos) 
    print ("weight ratio", weight)
    ...
    xgb_dtrain = xgb.DMatrix(data = train[:,:c], label = train[:,c])
    xgb_param = {'scale_pos_weight': weight,
            'learning_rate': 1,
            'max_depth': 3,
            'min_split_loss': 3,
    ...
    pickle.dump(xgb_model, open(savefile, "wb"))
...
def evaluate(name, model, test):
    ...
    test_data = test[:,:c]
    test_label = test[:,c]
    test_pred = model.predict(xgb.DMatrix(data=test_data))
    test_pred = np.array(list(map(lambda x:1 if x > 0.5 else 0, test_pred.tolist())))
